...
@section('title','Business Book Edit')
                            ...
                            <div class="row">
                                <div class="col-xl-12">
                                    <form method="post" class=""
                                          action="{{route('businessbook.update',$business_book->id)}}">
                                        {{method_field('PATCH')}}
                                        @csrf
                                                ...
                                                <select name="organization_branch_id" id="organization_branch_id"
                                                    ...
                                                    @foreach($organization_branches as $organization_branch)
                                                        <option value="{{$organization_branch->id}}"
                                                                @if($business_book->organization_branch_id==$organization_branch->id) selected @endif>{{$organization_branch->branch_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-8">
                                                <textarea name="notes" class="form-control" id="" cols="30"
                                                          rows="10">{{$business_book->notes}}</textarea>
                                            </div>
                                        </div>
